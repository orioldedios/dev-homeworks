...
bool j1App::Awake()
{
	bool ret = LoadConfig() && LoadSavesDoc();
...
bool j1App::LoadSavesDoc()
{
	...
	{
		savesnode = savesdoc.first_child();
	}
...
void j1App::RealLoad(){
	...
	for (item = modules.start; item != NULL; item = item->next)
	{
		...
		item->data->Load(savesnode.child(item->data->name.GetString()));
	}
}
...
void j1App::RealSave()const{
	...
	for (item = modules.start; item != NULL; item = item->next)
	{
		...
		item->data->Save(savesnode.child(item->data->name.GetString()));
	}
