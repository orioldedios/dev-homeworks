...
bool j1App::Awake()
{
	...
	pugi::xml_parse_result result = mydocument.load_file("config.xml");

	if (result) {
		mynode = mydocument.document_element();
	}
	else {
		LOG("ERROR\n");
	}
	...
	while(item != NULL && ret == true)
	{
		...
		pugi::xml_node* nodeptr = nullptr;

		if (!strcmp(item->data->name.GetString(),mynode.child(item->data->name.GetString()).name()))
		{
			nodeptr = &mynode.child(item->data->name.GetString());
		}
		

		ret = item->data->Awake(nodeptr);
		item = item->next;
	}
